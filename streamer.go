...
// Streamer handles all the clients and subscriptions.
...
func (streamer *Streamer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	d, _ = json.Marshal([2]string{"/unifrost/closed", "Client closed"})
...
// Subscribe subscribes the specified client to the specified topic.
	...
	go func() {
		...
		for {
			...
			if err != nil {
				...
				client.messageChannel <- message{event: "message", payload: string(d), messageType: ERROR}
			...
			client.messageChannel <- message{event: topic, payload: string(msg.Body), messageType: EVENT}
			msg.Ack()
		}
	}()
